...
load_dotenv() #to load all enviorment variables

genai.configure(api_key=os.getenv("GOOGLE_API_KEY"))

#create gemininpro response
def get_gemini_response(input):
    model=genai.GenerativeModel('gemini-pro')
    response = model.generate_content(input)
    return response.text

# Ensure you have set the OPENAI_API_KEY environment variable with your API key
...
st.text( "Compare Resume with JD")
